...
EXTERN SOHASH_BOOL_T 
mvfs_thr_sohash_verify_mth(
    sohash_entry_t *thr_hashentry, 
    void *args
);

void mvfs_thr_sohash_on_delete(sohash_entry_t *thr_hashentry);

struct mvfs_verify_mth_args {
    MVFS_THREADID_T threadid;
};

EXTERN int mvfs_lockless_thrhash_enabled;
#endif
...
mvfs_procinit(mvfs_cache_sizes_t *mma_sizes)
{
...
#ifdef MVFS_USE_SPLIT_ORDERED_HASH
    sohash_init_args_t *hashinit_args;
...
#ifdef MVFS_USE_SPLIT_ORDERED_HASH
    /* If the tunable mvfs_lockless_thrhash_enabled is not set then 
     * will have to revert to the old implementation of thread hash
     */
    if (mvfs_lockless_thrhash_enabled) {
        /* Allocate the structure that is required for initializing the split
         * ordered hash set.  
         */
        hashinit_args = (sohash_init_args_t *)
                        KMEM_ALLOC(sizeof(sohash_init_args_t), KM_SLEEP);
        if (hashinit_args == (sohash_init_args_t *)NULL) {
            mvfs_log(MFS_LOG_ERR,
                     "mvfs_procinit: no memory (sohash_init_args_t)");
            error = ENOMEM;
        }  

        if (error == 0) {
            /* Initialize the structure */
            MDKI_ATOMIC_SET_UINT32(&hashinit_args->sohash_num_segments, 
                                   SOHASH_SET_SEG_DEFAULT);
            hashinit_args->sohash_segment_sz = SOHASH_SET_SEG_DEFAULT;
            hashinit_args->sohash_keys_unique = SOKEY_UNIQUE_FALSE;
            hashinit_args->fn_verify_matched_entry = mvfs_thr_sohash_verify_mth;
            hashinit_args->fn_data_free = mvfs_thr_sohash_free_thread;
            hashinit_args->fn_compute_sohashkey = mvfs_thr_sohash_get_key;
            hashinit_args->fn_on_delete = mvfs_thr_sohash_on_delete;

            mcdp->proc_thr.mvfs_threadid_hashd =
                        sohash_init_hashtable(hashinit_args);

            if (mcdp->proc_thr.mvfs_threadid_hashd == NULL) {
                mvfs_log(MFS_LOG_ERR, 
                         "mvfs_procinit: Failed to init thread hash");
                error = ENOMEM;
            } else {
                mvfs_log(MFS_LOG_INFO,
                         "mvfs_procinit: sohash initialized. hashd 0x%x\n",
                         mcdp->proc_thr.mvfs_threadid_hashd);
            }

            KMEM_FREE(hashinit_args, sizeof(sohash_init_args_t));
        }
    } else {
        len = sizeof(mvfs_thread_t *) * MVFS_THREADHASH_SZ(mcdp);
        mcdp->proc_thr.mvfs_threadid_hashtable = 
                                    (mvfs_thread_t **)KMEM_ALLOC(len, KM_SLEEP);
        ...
        if (error == 0) {
            ...
            if (mvfs_splock_pool_init(&(mcdp->proc_thr.mvfs_threadid_spl_pool), 
                                      sp_poolsize, 
                                      NULL,
                                      "mvfs_thr_hash_spl") != 0)
            {
                mvfs_log(MFS_LOG_ERR,
                         "mvfs_procinit: no memory (threadhash locks)");
    ...
    if (error == 0) {
        ...
        mcdp->proc_thr.mvfs_procid_hashtable = 
                                (mvfs_proc_t **)KMEM_ALLOC(len, KM_SLEEP);
        if (mcdp->proc_thr.mvfs_procid_hashtable == (mvfs_proc_t **)NULL) {
            mvfs_log(MFS_LOG_ERR, "mvfs_procinit: no memory (prochash)");

#ifdef MVFS_USE_SPLIT_ORDERED_HASH
            if (mvfs_lockless_thrhash_enabled) {
                /* Cleanup all the data structures for split ordered hash set 
                 * initialized above.  
                 */
                if ((error = sohash_cleanup_hashtable(
                                mcdp->proc_thr.mvfs_threadid_hashd)) != 0) {
                    mvfs_log(MFS_LOG_ERR, 
                             "mvfs_procinit: Failed to cleanup thread hash. "
                             "error = %u\n", error);
                }
            } else {
                KMEM_FREE(mcdp->proc_thr.mvfs_threadid_hashtable,
...
mvfs_procdata_free()
{
    ...
    if (mvfs_lockless_thrhash_enabled) {
        if (mcdp->proc_thr.mvfs_threadid_hashd == NULL) return; 
    } else {
        if (mcdp->proc_thr.mvfs_threadid_hashtable == NULL) return;
    }
#else
    ...
    ASSERT(mcdp->proc_thr.mvfs_nproc_alloced == 0 && 
           mcdp->proc_thr.mvfs_nthr_alloced == 0);

#ifdef MVFS_USE_SPLIT_ORDERED_HASH
    if (mvfs_lockless_thrhash_enabled) {
        /* Cleanup the split ordered hash set */
        if (sohash_cleanup_hashtable(mcdp->proc_thr.mvfs_threadid_hashd) != 0) {
            mvfs_log(MFS_LOG_ERR, 
                     "mvfs_procdata_free: Failed to cleanup thread hash"); 
        }
    } else {
        KMEM_FREE(mcdp->proc_thr.mvfs_threadid_hashtable,
    	          sizeof(mvfs_thread_t *)*MVFS_THREADHASH_SZ(mcdp));
...
#ifdef MVFS_USE_SPLIT_ORDERED_HASH
SOHASH_KEY_T
mvfs_thr_sohash_get_key(void *mth)
{
    return(((mvfs_thread_t *)mth)->thr_sohash_key);
}

/* When an entry in the split ordered hash is deleted, the memory
 * corresponding to the entry might be released asynchronously.
 * mvfs_thr_sohash_free_thread() is registered as a call back 
 * for freeing the memory allocated for mvfs_thread_t, when the 
 * corresponding hash entry is being freed.  
 */
void 
mvfs_thr_sohash_free_thread(void *ptr)
{
    mvfs_thread_t *thr = (mvfs_thread_t *)ptr;
...
/* As the generated split ordered keys for MVFS thread hash are not unique,
 * the split ordered hash set implementation will invoke the call back routine,
 * mvfs_thr_sohash_verify_mth(), to let MVFS thread sub-system verify if the
 * hash entry that is found, is the one that is expected.  struct
 * mvfs_verify_mth_args contains the additional arguments required by the call 
 * back routine to make this comparison and is passed to the sohash 
 * find/insert/delete interfaces.  
 */
SOHASH_BOOL_T 
mvfs_thr_sohash_verify_mth(
    sohash_entry_t *thr_hashentry, 
...
{
    mvfs_thread_t *mth = (mvfs_thread_t *)(thr_hashentry->data);
    MVFS_THREADID_T threadid = ((struct mvfs_verify_mth_args *)args)->threadid;
...
/* The split ordered hash set implementation provides a way to register a call
 * back to perform any operation right after a hash entry has been deleted.
 * This is for calling mvfs_threadfree() to dequeue the thread from the 
 * mp_threads list of mvfs_proc struct.  Note that the hash implementation
 * could release the memory for the hash entry and the mvfs_thread_t
 * asynchronously.  This call back ensures that mvfs_threadfree() can be
 * called for the cleanup just after the thread structure is removed from the
 * hash but before it is freed up.  
 */
void 
mvfs_thr_sohash_on_delete(sohash_entry_t *thr_hashentry)
{
    mvfs_thread_t *thr = (mvfs_thread_t *)(thr_hashentry->data);

    ASSERT(thr->thr_sohash_entry == thr_hashentry);
...
mvfs_mythread_subr_sohash(MVFS_THREADID_T *threadid_in)
{
    ...
    register mvfs_proc_t *mpchase, **mpp;
    mvfs_common_data_t *mcdp = MDKI_COMMON_GET_DATAP();
    SOHASH_KEY_T thr_sohash_key;
    sohash_entry_t *thr_hashentry = NULL;
    struct mvfs_verify_mth_args verify_args;
    ...
    verify_args.threadid = threadid;
    /*
    verify_args.procid = mepid;
    verify_args.proctag = metag;
    */

    thr_hashentry = sohash_find_entry(mcdp->proc_thr.mvfs_threadid_hashd, 
                                      thr_sohash_key,
                                      (void *)(&verify_args));

    if ((thr_hashentry == NULL) && 
        (MDKI_ATOMIC_READ_UINT32(
             &mcdp->proc_thr.mvfs_threadid_hashd->hash_state) 
                 == SOHASH_STATE_FLUSH)) 
    {
        MDKI_PANIC("mythread: Detected thread hash flush"); 
    }
    
    if (thr_hashentry != NULL) {
        mth = (mvfs_thread_t *)(thr_hashentry->data);

	ASSERT((mth != NULL) && 
               (mth->thr_sohash_key == thr_sohash_key) && 
               (mth->thr_sohash_key == thr_hashentry->so_key));
	...
	if (MDKI_PROC_EQ(mth->thr_proc, &mepid, &metag)) {
        ...
        mvfs_log(MFS_LOG_DEBUG,
	         "mvfs_mythread: changing allegiance, mthr %"KS_FMT_PTR_T" "
    ...
    if (!(MDKI_PROC_EQ(mth->thr_proc, &mepid, &metag))) {
        mvfs_log(MFS_LOG_WARN,
		 "mvfs_mythread: different procinfo  mthr 0x%x procid 0x%x "
                 "proctag 0x%x mprocid 0x%x mproctag 0x%x\n",
		 mth, mepid, metag, mth->thr_proc->mp_procid, 
                 mth->thr_proc->mp_proctag);
    }
#endif
    ...
    if (!MDKI_IS_SOL_EXITPROC(mepid,mcdp)) {
        ...
        /* Insert the new thread in the hash */
        error = sohash_insert_entry(mcdp->proc_thr.mvfs_threadid_hashd,
                                    thr_sohash_key,
                                    (void *)mth,
                                    (void *)(&verify_args),
                                    &thr_hashentry);
        ...
        if (error != 0) {
            /* A thread structure was not found in the earlier search.  Error of
             * EEXIST from sohash_insert_entry() means that an entry was found
             * in the hash for the same key.  As this is thread specific 
             * structure and nobody else could be allocating a thread struct for
             * us, this is an unexpected error condition.  
             */
            if (error == EEXIST) {
                MDKI_PANIC("mythread: Insert in thread hash failed with EEXIST");
            }

            /* The callers of mvfs_mythread() assume that a valid thread 
             * structure is returned always.  
             */
            MDKI_PANIC("mythread: Failed to insert thread info in hash");
        } else {
            mth->thr_sohash_entry = thr_hashentry;
        }
    }
    ...
    if (mthproc != NULL) {
            ...
            !MVFS_PROCVALID(mthproc,mth))
        {
             ...
             * identified.
             */
            proc_hashindex = mvfs_pidhash(&mthpid);
            mpp = &(mcdp->proc_thr.mvfs_procid_hashtable[proc_hashindex]);
...
tbs_boolean_t needs_dequeue; /* Only dequeue thread if on mp_threads list */
{
    ...
    /* The split ordered hash implementation takes care of freeing
     * the memory allocated for the thread by calling the routine
     * (mvfs_thr_sohash_free_thread()) specified during the hash 
     * initialization.  Note that the freeing of memory could be 
     * asynchronous as per the Safe Memory Reclamation algorithm.  
     * Refer to mvfs_sohash_table.c for details.  
     * If mvfs_lockless_thrhash_enabled is set to zero then split 
     * ordered hash set is not enabled.   
     */
#ifdef MVFS_USE_SPLIT_ORDERED_HASH
    if (mvfs_lockless_thrhash_enabled == 0) {
#endif
        BZERO(thr, sizeof(*thr));
...
#ifdef MVFS_USE_SPLIT_ORDERED_HASH
/* mvfs_threadfree() is not directly called by mvfs_threadrele() in the thread
 * hash implementation using the split ordered hash sets.  It is
 * called via the call back routine, mvfs_thr_sohash_on_delete(), registered
 * with the split ordered hash implementation, after the thread info structure
 * is removed from the hash but before it is freed.  
 */
STATIC void
mvfs_threadrele_subr_sohash(thr)
mvfs_thread_t *thr;
{
    int error = 0;
    struct mvfs_verify_mth_args verify_args;
    ...
    verify_args.threadid = thr->thr_threadid;
    error = sohash_delete_entry(mcdp->proc_thr.mvfs_threadid_hashd,
                                thr->thr_sohash_key,
                                (void *)(&verify_args));
    if (error != 0) {
        /* If the hash is not being flushed (i.e state is SOHASH_STATE_ACTIVE)
         * and the delete failed with the error return, ENOENT, then 
         * thr_sohash_key was not found in the hash.  
         */
        if ((MDKI_ATOMIC_READ_UINT32(
                &mcdp->proc_thr.mvfs_threadid_hashd->hash_state) == 
                    SOHASH_STATE_ACTIVE) && 
            (error == ENOENT)) 
        {
            MDKI_PANIC("Thread not found in sohash");
        } 
        ...
        if ((MDKI_ATOMIC_READ_UINT32(
            ...
            (error == EFAULT)) 
        {
            MDKI_PANIC("sohash returned EFAULT on delete entry");
        }
    }
}

STATIC void
mvfs_threadrele(mvfs_thread_t *thr)
{
    if (mvfs_lockless_thrhash_enabled) {
        mvfs_threadrele_subr_sohash(thr);
    } else {
...
static const char vnode_verid_mvfs_procops_c[] = "$Id:  83354e7a.46fd11e3.8592.00:01:84:c3:8a:52 $";
