...
/* A custom error code to indicate that a CCRC operation failed because
   the server was too busy */
#define TBS_ST_CCRC_SERVER_BUSY TBS_ST_CCRC_BASE + 22

/* A custom error code to indicate that a CCRC operation failed because
   of character transcoding problems */
#define TBS_ST_CCRC_CHAR_ENCODING_FAILED TBS_ST_CCRC_BASE + 23

/* CCRC highest status code */
#define TBS_ST_CCRC_LIMIT                  TBS_ST_CCRC_BASE + 24
 ...
 * 
 * A little history is needed to explain the need for the following 3
 * tbs_fstat_*_t types.  The first one, tbs_fstat_t, is the original fstat
 * information kept by ClearCase for an object.  The second one,
 * tbs_fstat_db_t, was added to allow large files (size changed from u_long to
 * ks_off64_t) and to keep user credentials as SIDs rather than Unix ids (plus
 * a few other changes).  This required a schema roll and many new RPCs (for
 * view, vob, db, etc.) to take the new type.  The last one, tbs_fstat_full_t,
 * was added to carry along the ACL information for an object (in the form of
 * the rolemap dbid of the rolemap attached to the object).  Currently, this
 * last form is only used in libvob when creating the source container for a
 * new element, so its effects are limited.  That usage may change in the
 * future to become the "real" fstat information, which would involve changing
 * the same RPCs as mentioned above, and essentially replacing the
 * tbs_fstat_db_t with tbs_fstat_full_t.
 *
 ...
 *
 * tbs_fstat_full_t	File status with full security info
 * .ctime		Time of last status change.
 * .flags		Flags (including Windows attribute bits)
 * .gsid		Owner group id.
 * .mode		Protection mode bits.
...
typedef struct tbs_fstat_full_s {
    tbs_ftype_t		type;
    tbs_fmode_t		mode;
    tbs_dbid_t          rolemap_dbid;
    u_long		nlink;
    u_long		flags;
    credutl_sid_t	usid;
    credutl_sid_t	gsid;
