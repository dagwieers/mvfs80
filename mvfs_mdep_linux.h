...
#define MDKI_THREAD_SOHASH_KEY(tagp) \
...
/* Atomically compare and swap unsigned int */
#define MDKI_ATOMIC_CAS_UINT32(addr, cmpval, newval) \
    ((atomic_cmpxchg((addr), (cmpval), (newval)) == (cmpval)) ? 1 : 0)

/* Atomically compare and swap pointer value */
#define MDKI_ATOMIC_CAS_PTR(addr, cmp_addr, new_addr) \
    ((cmpxchg((addr), (void *)(cmp_addr), (void *)(new_addr)) == (cmp_addr)) ? 1 : 0)

/* Atomically compare and swap unsigned long */
#define MDKI_ATOMIC_CAS_ULONG(addr, cmpval, newval) NOTIMPLEMENTED()

/* Atomically increment and decrement unsigned int */
#define MDKI_ATOMIC_INCR_UINT32(addr)  atomic_inc(addr)
#define MDKI_ATOMIC_DECR_UINT32(addr)  atomic_dec(addr)

/* Atomically increment unsigned integer and return new value */
#define MDKI_ATOMIC_INCR_UINT32_NV(addr) atomic_inc_return(addr)

/* Macros for atomic set and atomic read */
#define MDKI_ATOMIC_SET_UINT32(addr, val)     atomic_set((addr), (val))
#define MDKI_ATOMIC_READ_UINT32(addr)         atomic_read(addr)
#define MDKI_ATOMIC_PTR_READ(addr)     (*((typeof(*(addr)) *)(addr)))
#define MDKI_ATOMIC_PTR_SET(addr, val) \
    ((*((volatile typeof(*(addr)) *)(addr))) = (val))

#endif /* MVFS_MDEP_LINUX_H_ */
/* $Id: d26cf0c8.1fb911e2.96af.00:01:84:c3:8a:52 $ */
