...
/* The lockless split ordered hash set implementation is used for 
 * thread hash on some of the platforms (if MVFS_USE_SPLIT_ORDERED_HASH
 * is defined).  Older thread hash implementation can be reverted back
 * to by setting the mvfs_lockless_thrhash_enabled tunable to 0. Hence
 * the definition for mvfs_thread on platforms where split ordered hash
 * set can be enabled has members that are needed by the older 
...
#ifdef MVFS_USE_SPLIT_ORDERED_HASH
    /* Following fields will be used only when the split ordered hash set 
     * implementation is used for thread hash.  Note: If the split ordered
     * hash set implementation is used then fields thr_hashnxt and
     * thr_hashbucket will not be used.
     */
    SOHASH_KEY_T               thr_sohash_key; /* Split ordered hash key */
    sohash_entry_t             *thr_sohash_entry; /* sohash entry for the above key */ 
#endif /* MVFS_USE_SPLIT_ORDERED_HASH */
};
...
/* The lockless split ordered hash set implementation is used for 
 * thread hash on some of the platforms (i.e. if MVFS_USE_SPLIT_ORDERED_HASH
 * is defined).  Older thread hash implementation can be reverted back
 * to by setting the mvfs_lockless_thrhash_enabled tunable to 0.
 */
typedef struct mvfs_proc_thread_data {
...
#ifdef MVFS_USE_SPLIT_ORDERED_HASH
    sohash_table_t *mvfs_threadid_hashd; /* Split ordered hash descriptor */
#endif
    /*  
     ...
     * 
     * NOTE: mvfs_threadid_spl_pool and mvfs_threadid_hashtable will be used
     * only if the use of split ordered hash set implementation for thread
     * hash is disabled (i.e. mvfs_lockless_thrhash_enabled is set to 0)
     */
...
** for the no-vp case.
*/
#define MVFS_CHK_STALE_MNP(err, mnp, vfsp, cd)                          \
    if (MTOV(mnp) != NULL) {                                            \
...
#define MVFS_STAT_MEMALLOC1 { \
    MVFS_SAVE_INTR_T orig_intr_level; \
    int cur_cpuid; \
