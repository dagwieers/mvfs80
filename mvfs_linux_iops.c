...
/* On getxattr reiserfs gets the lock but ext3 doesn't.
 * Neither get the lock on listxattr.
 * I get the lock in both cases here in case it matters to some other fs.
 * NOTE: This code is seriously broken.  it uses RATL_SUSE as a proxy for
 * checking for reiserfs.  (Of course there is no way to check for this at
 * compile time.) As of SLES11, reiserfs is no longer the SUSE default.
 * Fortunately the xattr functions are rarely called.
 */
extern ssize_t
vnode_iop_getxattr(
    struct dentry *dentry,
