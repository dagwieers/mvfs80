...
typedef struct view_eacl_cursor_s {
    struct timeval eacl_mtime;
    ks_uint32_t next_offset;
} view_eacl_cursor_t;
#define VIEW_EACL_CURSOR_NULL {{0,0},0}
#define VIEW_EACL_CURSOR_IS_NULL(cursor_p) ((cursor_p)->next_offset == 0)
#define VIEW_EACL_CURSOR_RESET(cursor_p) (cursor_p)->next_offset = 0, (cursor_p)->eacl_mtime.tv_sec = 0, (cursor_p)->eacl_mtime.tv_usec = 0
...
/*------------------------------------------------------------------------------
 * View stat         A rich stat structure that describes clearcase file
 * structure         objects.
 *
 * view_vstat_t
 *  .fstat 		tbs_fstat_db_t structure of the associated object.
 *  .mtype		Meta-type of associated object.
 *  .elem_oid		Element ID of object associated with the file if it
 *			is a versioned object, else TBS_OID_NULL.
 *  .obj_oid		Object ID of object associated with the file.
 ...
 *			necessary.
 *  .rolemap_oid        Rolemap ID of object associated with the file if it
 *                      is a versioned object, else TBS_OID_NULL.
 *  .eacl_mtime         Time of last update to the effective ACL generated by
 *                      the rolemap.
 */
 ...
 */
/*------------------------------------------------------------------------------
 * View stat v8      A rich stat structure that describes clearcase file
 * structure         objects.
 *
 * view_vstat_t
 *  .fstat 		tbs_fstat_db_t structure of the associated object.
...
#define VIEW_VSTAT_V8_TO_VIEW_VSTAT(vs8,vs)     \
    (vs)->fstat = (vs8)->fstat,                 \
    (vs)->mtype = (vs8)->mtype,                 \
    (vs)->elem_oid = (vs8)->elem_oid,           \
    (vs)->obj_oid = (vs8)->obj_oid,             \
    (vs)->event_time = (vs8)->event_time,       \
    (vs)->rolemap_oid = TBS_OID_NULL,           \
    (vs)->eacl_mtime.tv_sec = 0,                \
    (vs)->eacl_mtime.tv_usec = 0
#define VIEW_VSTAT_TO_VIEW_VSTAT_V8(vs,vs8)     \
    (vs8)->fstat = (vs)->fstat,                 \
    (vs8)->mtype = (vs)->mtype,                 \
    (vs8)->elem_oid = (vs)->elem_oid,           \
    (vs8)->obj_oid = (vs)->obj_oid,             \
    (vs8)->event_time = (vs)->event_time
/*
...
#define VIEW_NAME_STATE_NOACLPERM       (0x00100000L) /* ACLs says no permission */
...
typedef enum view_invalidate_type {
    ...
    VIEW_INVALIDATE_TYPE_ROLEMAP, /* Invalidate a particular rolemap.
                                     The replica_oid and obj_oid name need to be
                                     filled in.  The obj_oid represents the oid
                                     of the rolemap. */
    VIEW_INVALIDATE_TYPE_POLICY  /* Invalidate a particular policy and all
                                    its rolemaps.
                                    The replica_oid and obj_oid name need to be
                                    filled in.  The obj_oid represents the oid
                                    of the policy. */
...
#endif /* VIEW_BASE_H */
/* $Id: 649747dd.569111e2.8db0.00:01:83:9c:f6:11 $ */
